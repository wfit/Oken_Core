local _, Oken = ...
local Token = Oken:RegisterModule("Token", "AceTimer-3.0")
...
local token_config = {
	...
	cmds = Oken.Config:MakeDoc("Available chat commands", 1900, {
	...
	token = Oken.Config:MakeDoc("Token API", 3000, {
...
function Token:OnInitialize()
	Network = Oken.Network
	Roster = Oken.Roster
	Console = Oken.Console

	Console:RegisterCommand("token", self)
	Oken.Config:Register("Service token", token_config)
end
...
function Token:OnEnable()
	...
	self:RegisterMessage("OKEN_MSG_TKN2")
...
function Token:OKEN_MSG_TKN2(_, data, channel, sender)
	...
	if data[MSG_PACKED] then
		...
		for _, item in ipairs(data[MSG_PACKED]) do
			...
			self:OKEN_MSG_TKN2(nil, item, channel, sender)
		end
...
function TokenObj:Disable(unavailable, no_release)
	if self.state ~= STATE_DISABLED and self.state ~= STATE_DISPOSED then
		...
		if self:IsMine() then
			Token:SendMessage("OKEN_TOKEN_LOST", self.name, self)
		end
...
function TokenObj:SetOwner(ownerid, name)
	...
	if ownerid[K_GUID] ~= self.last_owner then
		if was_mine then Token:SendMessage("OKEN_TOKEN_LOST", self.name, self) end
		Token:SendMessage("OKEN_TOKEN_ACQUIRED", self.name, self)
		if is_mine then Token:SendMessage("OKEN_TOKEN_WON", self.name, self) end
	end
