local _, Oken = ...
local Cooldowns = Oken:RegisterModule("Cooldowns")

local OkenCooldownsTooltip = OkenCooldownsTooltip
local Roster
...
local cooldowns_config = {
	...
	unit_api = Oken.Config:MakeDoc("Unit API", 2100, {
	...
	cd_api = Oken.Config:MakeDoc("Cooldown API", 2200, {
...
function Cooldowns:OnInitialize()
	Oken.Config:Register("Cooldowns tracker", cooldowns_config)
	Roster = Oken.Roster

	self.db = Oken.db:RegisterNamespace("Cooldowns", cooldowns_default)
	self.settings = self.db.profile
end

function Cooldowns:OnEnable()
	self:RegisterMessage("OKEN_ROSTER_UPDATE")
	self:RegisterMessage("OKEN_ROSTER_LEFT")
	self:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED")
	self:RegisterEvent("ENCOUNTER_START")
	self:RegisterEvent("ENCOUNTER_END")
	self:RegisterMessage("OKEN_MSG_COOLDOWNS")
end
...
function Cooldown:Update()
	...
	self:Emit("OKEN_COOLDOWNS_UPDATE")
end
...
function Cooldown:BeginCooldown(cooldown)
	...
	self:Emit("OKEN_COOLDOWNS_START", cooldown)
end
...
function Cooldown:CooldownFinished()
	...
	self:Emit("OKEN_COOLDOWNS_READY")
...
function Cooldown:Trigger(target, spell, cooldown)
	if self:Invoke("ontrigger", target, spell, cooldown) ~= false then
		...
		self:Emit("OKEN_COOLDOWNS_USED", duration, target, spell)
...
function Cooldown:Reset()
	...
	if should_reset and self:Invoke("onreset") ~= false then
		...
		self.cooldown = 0
		self:Emit("OKEN_COOLDOWNS_RESET")
	end
end

function Cooldown:Dispose()
	self:CancelTimer()
	self:Emit("OKEN_COOLDOWNS_LOST")
			...
			if not cd.spell.encounter then
				...
				elseif not cd.spell.nocheck then
					do -- Cooldown
						...
						OkenCooldownsTooltip:ClearLines()
						OkenCooldownsTooltip:SetSpellByID(id)

						local found = false
						for i = 2, OkenCooldownsTooltip:NumLines() do
							local text = _G["OkenCooldownsTooltipTextRight" .. i]
...
function Cooldowns:OKEN_ROSTER_UPDATE(_, guid, _, info)
	self:UpdateUnit(guid, info)
end

function Cooldowns:OKEN_ROSTER_LEFT(_, guid)
	local unit = self.units[guid]
	if unit then
		unit:Dispose()
		self.units[guid] = nil
...
do
	...
	local function SpellCasted(source, target, spell, broadcast)
		...
		if broadcast and IsInGroup() then
			Oken:Send("COOLDOWNS", { source = source, target = target, spell = spell, key = key }, "RAID")
		end
	end
	...
	function Cooldowns:OKEN_MSG_COOLDOWNS(_, data)
